...
d <- compareProjectToStoredOutputFiles(projectPaths, ignore = c("MinLayerDepth", "MaxLayerDepth", "Layer"), data.out = TRUE, tolerance = 1e-12)
...
oldStratumArea <- data.table::as.data.table(read.table(unz(projectPaths, "coastalCod_20/output/baseline/StratumArea/StratumAreaData.txt"), sep = "\t", header = TRUE))
