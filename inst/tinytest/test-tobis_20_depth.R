...
expect_true("ReportBootstrapByIndividualAge.ReportBootstrapData" %in% names(d$test$data_equal))
...
oldStratumArea <- data.table::as.data.table(read.table(unz(projectPaths, "tobis_20_depth/output/baseline/StratumArea/StratumAreaData.txt"), sep = "\t", header = TRUE))
